...
import useMintCallback, { MintCallbackState } from 'hooks/useMintCallback'
...
export default function Mint() {
  ...
  const [
    showApprove1, showApproveLoader1,
    showApprove2, showApproveLoader2,
  ] = useMemo(() => {
    const show1 = approvalState1 !== ApprovalState.APPROVED
    const show2 = Token2 && approvalState2 !== ApprovalState.APPROVED
    return [
      show1, show1 && approvalState1 === ApprovalState.PENDING,
      show2, show2 && approvalState2 === ApprovalState.PENDING,
    ]
  ...
  function getApproveButton(): JSX.Element | null {
    ...
    if (showApprove1) {
      return (
        <PrimaryButton onClick={handleApprove1}>
          Allow DEUS to spend your {Token1?.symbol}
        </PrimaryButton>
      )
    }
    if (showApprove2) {
      return (
        <PrimaryButton onClick={handleApprove2}>
          Allow DEUS to spend your {Token2?.symbol}
        </PrimaryButton>
      )
    }
    return null
  ...
  function getActionButton(): JSX.Element | null {
    ...
    if (showApprove1 || showApprove2) {
      return (
        <PrimaryButton disabled>Mint DEI</PrimaryButton>
      )
    }
    if (error) {
      return (
        <PrimaryButton disabled>Critical Error</PrimaryButton>
      )
    }
    ...
    return (
      <PrimaryButton onClick={() => amountOut && dispatch(setShowReview(true))}>
        Mint DEI
      </PrimaryButton>
    )
  }
  ...
  function getMainContent(): JSX.Element {
    if (!account || !chainId) {
      return (
        <TextBlock>Please connect your wallet.</TextBlock>
      )
    }
    if (!isSupportedChainId) {
      return (
        <TextBlock>Please connect with one of our supported chains (see above).</TextBlock>
      )
    }
    return (
      <>
