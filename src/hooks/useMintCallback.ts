...
export default function useMintCallback(
  ...
  const oracleRequest = useCallback(async (query) => {
    try {
  ...
  const getParamsNoProxy = useCallback(async() => {
    try {
      if (collateralRatio == 1) {
        ...
        return [
          'mint1t1DEI',
          [amount1BN, collateral_price, expire_block, signature],
        ]
      }
      if (collateralRatio > 0) {
        ...
        return [
          'mintFractional',
          [amount1BN, amount2BN, collateral_price, deus_price, expire_block, signature],
        ]
      }
  ...
  const getParamsProxy = useCallback(async(method) => {
    try {
      ...
      const path: string[] | null = chainId && Token1
        ? MINT__PATHS[chainId][Token1.symbol]
        : null
  ...
  }, [proxyValues, Token1, amount1BN, amountOutBN, minimumAmountOutBN])

  const constructCall = useCallback(async () => {
    try {
      const Contract = isProxyMinter
      ? ProxyMinterContract
      : CollateralPoolContract
      
      if (!chainId || !Token1 || !Contract) {
        throw new Error('Missing dependencies.')
      }

      const methodName = Token1.address == Collateral[chainId]
        ? CallMethods.USDC
        : Token1?.address == '0x'
          ? CallMethods.NATIVE
          : CallMethods.ERC20

      const args = isProxyMinter
        ? await getParamsProxy(methodName)
        : await getParamsNoProxy()
  ...
  return useMemo(() => {
    ...
    return {
      ...
      callback: async function onMint(): Promise<string> {
        ...
        const { address, calldata, value, staticResult: exactDeiOut }: MintCall = call
